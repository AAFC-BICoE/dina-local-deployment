...
| Docker Compose CLI plugin | v5.0.1
| Node.js (via nvm) | 20.x
| nvm | v0.40.2
| Yarn (Corepack-managed) | 1.22.21
| Maven | 3.9.8
|===

== Prerequisites

Linux operating system (Virtual Machine). Tested with Ubuntu {var_ubuntu_version}.
...
----
apt-cache policy docker-ce | grep {var_docker_version}
----
...
[source, bash]
----
apt-get install docker-ce=5:25.0.1-1~ubuntu.22.04~jammy docker-ce-cli=5:25.0.1-1~ubuntu.22.04~jammy containerd.io
----
...
[source, bash]
----
git clone --branch dev https://github.com/AAFC-BICoE/dina-local-deployment.git
----
