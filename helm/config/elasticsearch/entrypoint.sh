...
if [ -f /usr/share/elasticsearch/config/certs/ca/ca.crt ] && [ -f /usr/share/elasticsearch/config/certs/elastic-node-certificates.p12 ]; then
  ...
  elasticsearch-certutil cert --name elastic-certs --ca-cert /usr/share/elasticsearch/config/certs/ca/ca.crt --ca-key /usr/share/elasticsearch/config/certs/ca/ca.key --pem --dns 'elasticsearch-service' --days 3650 --out /usr/share/elasticsearch/config/certs/elastic-certs.zip
